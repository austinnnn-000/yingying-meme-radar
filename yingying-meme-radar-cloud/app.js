...
function mapDexPairs(pairs) {
  ...
  return pairs
    ...
    .map((pair) => ({
      ...
      tags: [pair.chainId, pair.dexId, pair.baseToken.name].filter(Boolean),
      url: pair.url,
    }));
}
...
function filteredTokens() {
  ...
  return state.tokens
    ...
    .sort((a, b) => {
      if (state.sort === "watch") return Number(state.watchlist.has(b.symbol)) - Number(state.watchlist.has(a.symbol)) || b.heat - a.heat;
...
function renderTokens() {
  ...
  tokens.forEach((token) => {
    ...
    row.className = `token-row ${state.selected?.symbol === token.symbol ? "selected" : ""}`;
    row.type = "button";
    ...
    row.innerHTML = `
      ...
      <span class="watch-button ${state.watchlist.has(token.symbol) ? "active" : ""}" data-watch="${token.symbol}" title="自选">★</span>
      <span class="token-name">
        <strong>${token.pair}</strong>
        <span>${token.chain} · ${token.dex} · ${token.age}</span>
      ...
      <span class="token-stat"><span>24h</span><strong>${token.change > 0 ? "+" : ""}${token.change.toFixed(1)}%</strong></span>
...
function renderWatchlist() {
  const watched = [...state.watchlist].filter((symbol) => state.tokens.some((token) => token.symbol === symbol));
  if (!watched.length) {
  ...
  watched.forEach((symbol) => {
    ...
    chip.addEventListener("click", () => selectToken(symbol));
...
function toggleWatch(symbol) {
  if (state.watchlist.has(symbol)) {
    ...
    addEvent("news", `${symbol} 移出自选`, "本地自选已更新");
  } else {
    state.watchlist.add(symbol);
    addEvent("hot", `${symbol} 加入自选`, "后续异动会更醒目");
  }
...
function selectToken(symbol) {
  const token = state.tokens.find((item) => item.symbol === symbol);
  if (!token) return;
  state.selected = token;
  renderTokens();
  renderDetail();
...
function simulateTick() {
  ...
  if (candidate.heat >= state.heatThreshold) {
    const prefix = state.watchlist.has(candidate.symbol) ? "自选 " : "";
    addEvent("hot", `${prefix}${candidate.symbol} 热度突破 ${candidate.heat}`, `24h ${candidate.change.toFixed(1)}% · 成交 $${formatCompact(candidate.volume)}`);
    notify(candidate);
  ...
  if (state.selected) {
    state.selected = state.tokens.find((token) => token.symbol === state.selected.symbol) || state.selected;
  }
  renderAll();
}
...
function notify(token) {
  ...
  new Notification(`嘤嘤雷达：${token.symbol} 异动`, {
    body: `热度 ${token.heat} · 24h ${token.change.toFixed(1)}% · 成交 $${formatCompact(token.volume)}`,
  });
}
...
async function refreshData() {
  ...
  const [remoteTokens, news] = await Promise.all([fetchDexPairs().catch(() => null), fetchNews()]);
  state.news = news;
  state.tokens = (remoteTokens || fallbackTokens).map(decorateToken);
